...
async def info_cmd(update, context):
    if update.effective_user.id != OWNER_ID:
        return
    if not context.args:
        return await update.message.reply_text("Usage: /info USER_ID")
...
async def removeall_cmd(update, context):
    ...
    if target in d["blacklist"]:
        return await update.message.reply_text(f"⛔ {target} blacklist me hai. Pehle /unban karo.")
...
async def ban_cmd(update, context):
    ...
    if target not in d["blacklist"]:
        d["blacklist"].append(target)
        d["total_bans"] += 1
        save_data(d)
        add_log(f"Ban: {target}")
...
async def unban_cmd(update, context):
    ...
    d = load_data()
    if target in d["blacklist"]:
        d["blacklist"].remove(target)
        save_data(d)
        add_log(f"Unban: {target}")
